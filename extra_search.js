function earlyBtnEventHandler(event) {
  blockLoading();
  console.log("earlyBtnEventHandler ;;;;;  ");
  ...
  function checkRentableDate(srchLastDay, useDt, rsrvtCycle) {
    var rsrvtCycleDay = rsrvtCycle === "MON" ? 31 : 7;
    ...
    var day3 = new Date(
      +srchLastDay.substring(0, 4),
      +srchLastDay.substring(4, 6) - 1,
      +srchLastDay.substring(6, 8) + rsrvtCycleDay
    );

    return day1.getTime() < day2.getTime() && day2.getTime() <= day3.getTime();
  }
  const rsrvtCycle = monthRsrvtStatus.data.rsrvtPolcy.rsrvtCycleTpeCd;
  monthRsrvtStatus.goodsList.forEach((goods, index) => {
    ...
    goods.forEach((good, index) => {
      ...
      if (
        goodsTd &&
        checkRentableDate(monthRsrvtStatus.srchLastDay, useDt, rsrvtCycle)
      ) {
